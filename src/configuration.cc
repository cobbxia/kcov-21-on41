...
public:
	...
	bool parse(unsigned int argc, const char *argv[])
	{
		...
		while (1) {
			...
			case 'p':
				if (!isInteger(std::string(optarg)))
                                {
                                        std::cout<<std::string(optarg)+"not integer\n";
					return usage();
                                }
				m_ptracePid = stoul(std::string(optarg));
                                std::cout<<"pid:"+std::string(optarg)+"\n";
				extraNeeded = 1;
				break;
			case 't':
				m_title = optarg;
			...
			default:
                ...
                std::cout<<"afterOpts:"<<afterOpts<<"\textraNeeded:"<<extraNeeded<<"\targc:"<<argc<<"\n";
		...
		if (argc < afterOpts + extraNeeded)
                {
                        std::cout<<"\nafterOpts:"<<afterOpts<<"\textraNeeded:"<<extraNeeded<<"\targc:"<<argc<<"\n";
			return usage();
                }
