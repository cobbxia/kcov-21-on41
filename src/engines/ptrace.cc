...
public:
	...
	~Ptrace()
	{
		...
		kill(SIGTERM);
		ptrace((__ptrace_request)PTRACE_DETACH, m_activeChild, 0, 0);
	}
	...
	const Event waitEvent()
	{
		...
		if (WIFSTOPPED(status)) {
			...
			ptrace((__ptrace_request)PTRACE_GETSIGINFO, m_activeChild, NULL, (void *)&siginfo);
			...
			if (sig == SIGTRAP || sig == SIGSTOP) {
				...
				return out;
			} else if ((status >> 16) ==((__ptrace_request)PTRACE_EVENT_CLONE || (status >> 16) == (__ptrace_request)PTRACE_EVENT_FORK)) {
	...
	bool continueExecution()
	{
		...
		res = ptrace((__ptrace_request)PTRACE_CONT, m_activeChild, 0, m_signal);
		if (res < 0) {
...
private:
	bool forkChild(const char *executable)
	{
		...
		if ((child = fork()) == 0) {
			...
			res = ptrace((__ptrace_request)PTRACE_TRACEME, 0, 0, 0);
			if (res < 0) {
		...
		ptrace((__ptrace_request)PTRACE_SETOPTIONS, m_activeChild, 0, PTRACE_O_TRACECLONE | PTRACE_O_TRACEFORK);
	...
	bool attachPid(pid_t pid)
	{
		...
		errno = 0;
		ptrace((__ptrace_request)PTRACE_ATTACH, m_activeChild, 0, 0);
		if (errno) {
